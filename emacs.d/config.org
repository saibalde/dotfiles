...
I use `doom-themes` for a decent collection of color schemes, `telephone-line`
for a simple [[https://github.com/vim-airline/vim-airline][vim-airline]] like mode line, and `dimmer` to fade out inactive
windows when editing files in split view.
